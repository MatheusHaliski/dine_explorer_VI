...
export async function POST(request: NextRequest, context: { params: Promise<{ restaurantId: string }> }): Promise<Response> {
  ...
  try {
    ...
    const payload = { createdAt, grade: ratingValue, rating: ratingValue, restaurantId, text, userDisplayName, userEmail: userEmail || null };
    const docRef = await db.collection(COLLECTIONS.RESTAURANTS).doc(restaurantId).collection(SUB.REVIEWS).add(payload);
